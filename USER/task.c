...
Tcb_t *taskReadyList[maxPriority] = {NULL};
...
Tcb_t *findTaskHighest(Tcb_t *taskReadyList[]) {
	int i = 0;
    // 从最高优先级往下找（假设优先级数值越小优先级越高）
    for ( i = 0; i < maxPriority; i++) {
        if (taskReadyList[i] != NULL) {
            // 找到第一个非空链表，返回链表中优先级最高的任务
            // 因为同一个链表中的任务优先级相同，直接返回第一个即可
            return taskReadyList[i];
        }
    }
    return NULL;  // 没有就绪任务
}
...
void vTaskSwitchContext(void) {
    ...
    if (pxCurrentTcb != NULL && pxCurrentTcb->status == TASK_RUNNING) {
        ...
        taskListAdd(pxCurrentTcb,&taskReadyList[pxCurrentTcb->taskPriority]);
    }
...
Tcb_t *taskCreate(Tcb_t *tcb, uint32_t priority, uint32_t *stackArray, uint32_t size, void (*taskFunc)(void)) {
    ...
    *(--sp) = 0x04040404;              // R4
    ...
    taskListAdd(tcb,&taskReadyList[priority]);
    return tcb;
}
